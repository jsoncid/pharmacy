...
const COLLECTION_ID = import.meta.env.VITE_APPWRITE_COLLECTION_CATEGORIES;
...
export default function Categories() {
  ...
  const isNotEditable =
    selectedCategory?.editable === false ||
    selectedCategory?.editable === "false";
  ...
  const handleCreateSubmit = async () => {
    ...
    try {
      ...
      setCategories((prev) => [newDoc as unknown as Category, ...prev]);
      closeAllModals();
    } catch (err) {
      setFormError(
      ...
      setCategories((prev) =>
        prev.map((category) =>
          category.$id === selectedCategory.$id
            ? (updatedDoc as unknown as Category)
            : category,
        ),
      );
      ...
      >
        <div className="space-y-4">
          ...
          <p className="text-sm text-gray-600 dark:text-gray-300">
            Are you sure you want to deactivate this category? It will be hidden
