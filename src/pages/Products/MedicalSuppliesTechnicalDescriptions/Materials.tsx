...
const COLLECTION_ID = import.meta.env.VITE_APPWRITE_COLLECTION_MATERIALS;
...
export default function Materials() {
  ...
  const isNotEditable =
    selectedMaterial?.editable === false ||
    selectedMaterial?.editable === "false";
  ...
  const handleCreateSubmit = async () => {
    ...
    try {
      ...
      setMaterials((prev) => [newDoc as unknown as Material, ...prev]);
      closeAllModals();
    } catch (err) {
      setFormError(
      ...
      setMaterials((prev) =>
        prev.map((material) =>
          material.$id === selectedMaterial.$id
            ? (updatedDoc as unknown as Material)
            : material,
        ),
      );
    ...
    try {
      ...
      setFormError(
        err instanceof Error
          ? err.message
          : "Failed to deactivate material record",
      );
    } finally {
      setSubmitting(false);
    }
  ...
  const filteredMaterials = materials.filter((material) => {
    ...
    return material.description.toLowerCase().includes(query);
  });

  const totalPages = Math.max(
    1,
    Math.ceil(filteredMaterials.length / itemsPerPage) || 1,
  );
  ...
  return (
    <>
      <PageMeta
        title="Medical Supply Materials"
        description="Manage technical descriptions for medical supply materials."
      />
          ...
          >
            <div className="space-y-2">
              ...
              <InputField
                ...
                placeholder="Short description of material"
              />
            </div>
            {formError && (
              <p className="text-sm text-error-500">
      ...
      >
        <div className="space-y-4">
          ...
          <p className="text-sm text-gray-600 dark:text-gray-300">
            Are you sure you want to deactivate this material record? It will be
